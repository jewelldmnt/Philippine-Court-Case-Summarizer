...
#
# Purpose:
#     This program is designed to evaluate the performance of an AI-based text segmentation system by 
#     comparing its segmentation output against a reference (human) segmentation. The evaluation is performed 
#     using two metrics: WindowDiff and Pk score. The segmentation is based on boundaries marked by labels 
#     such as 'FACTS:', 'ISSUES:', and 'RULINGS:', which are used to define the segments in the text.
#
# Where the program fits in the general system design:
#     This program is part of a larger system aimed at evaluating AI-generated segmentations in natural language 
#     processing (NLP) tasks. It processes segmentation results, compares them to reference segmentations, 
#     and computes relevant metrics to measure the accuracy of AI segmentation models. It is useful in applications 
#     like legal document segmentation, where structured text categories are important for further analysis.
#
# Data Structures, Algorithms, and Control:
#     - Data Structures:
#         - **List (`segmentation`)**: Stores the segmentation boundaries (0 or 1), where 1 indicates a segmentation 
#           boundary and 0 indicates no boundary. The list is generated based on the presence of specific labels in the input.
#         - **List (`reference_boundaries`, `hypothesis_boundaries`)**: The boundaries are grouped into segments of 
#           consecutive 0s or 1s. These lists represent the reference and predicted segment lengths.
#     - Algorithms:
#         - **Segmentation Loading**: The `load_segmentation_with_labels` function loads segmentation data from a file 
#           and identifies boundaries using specific labels ('FACTS:', 'ISSUES:', 'RULINGS:').
#         - **Segmentation Evaluation**: The `evaluate_segmentation` function calculates the WindowDiff and Pk scores 
#           using the `segeval` library, which compares the reference segmentation against the AI's segmentation.
#     - Control:
#         - The program follows a clear step-by-step process:
#           1. It loads the segmentation data for both the human reference and AI predictions from text files.
#           2. It computes the segmentation evaluation metrics using the `evaluate_segmentation` function.
#           3. It outputs the WindowDiff and Pk scores, which help assess the quality of the AI-generated segmentation.
# =============================================================================
