...
def scrape_court_case(url):
    ...
    title = doc.find_all("h3")[0]
    decision_text = title.find_all("p")[-1].text.replace(" ", "")
    title.find_all("p")[-1].extract()
    ...
    if content.find("hr"):
        for sibling in content.find("hr").find_next_siblings():
            sibling.extract()

    new_content = BeautifulSoup(str(content), "html.parser")
    new_content = re.sub(r'\[(?:\bx\s+)+x\b\]', '', new_content.text)
    new_content = re.sub(r'(?:\b.\s+)+x\b', '', new_content)
    new_content = re.sub(r'\.{2,}', '', new_content)
    new_content = re.sub(r'…', '', new_content)
    new_content = re.sub(r'. . . .', '', new_content)
    new_content = re.sub(r'x{2,}', '', new_content)
    new_content = re.sub(r'X{2,}', '', new_content)

    sliced_content = new_content.strip()
