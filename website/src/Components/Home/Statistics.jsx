...
const Statistics = () => {
  ...
  const handleFileClick = (file) => {
    ...
    // Calculate unigrams and bigrams directly in the frontend
    const text = file.file_text.toLowerCase().replace(/[^a-z\s]/g, "");

    // Filter out stopwords, words less than or equal to 3 characters, and split into words
    const words = text
      .split(/\s+/)
      .filter((word) => !stopwords.includes(word) && word.length > 3);
    const unigrams = calculateUnigrams(words);
    const bigrams = calculateBigrams(words);
  ...
  const calculateBigrams = (words) => {
    const freqMap = {};
    for (let i = 0; i < words.length - 1; i++) {
      const bigram = `${words[i]} ${words[i + 1]}`;
      freqMap[bigram] = (freqMap[bigram] || 0) + 1;
    }
    const bigrams = Object.entries(freqMap).map(([bigram, frequency]) => ({
      bigram,
      frequency,
    }));
    return bigrams.sort((a, b) => b.frequency - a.frequency).slice(0, 10);
  };
    ...
    <div className="bg-customGray text-black h-screen">
      ...
      <div className="grid grid-cols-[1fr,2fr,2fr] gap-x-10 h-fit m-8">
            ...
            <table className="table-fixed w-full">
              ...
              <tbody>
                {wordStatsList.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan="2" className="text-sm text-center font-sans">
                      No data available
                    </td>
                  </tr>
                )}
            ...
            <table className="table-fixed w-full">
              ...
              <tbody>
                {bigramStatsList.length > 0 ? (
                  bigramStatsList.map((stat, index) => (
                    <tr key={index}>
                      <td className="text-sm text-center font-sans">
                        {stat.frequency}
                      </td>
        ...
        <div className="flex flex-col space-y-6">
          ...
          <div className="bg-customRbox rounded-xl w-full h-full max-h-[300px] p-4 wordcloud-container">
